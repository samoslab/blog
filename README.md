...
If there are no problems, then https://blog.Samos.io/ will automatically update in a few minutes.
...
For example, to add a German translation of `content/statement/Samos Distribution.md`,
name the file `content/statement/Samos Distribution.de.md`.

Themes: Layout and Styling
==========================

Samos Blog uses a custom hugo theme with styling produced using SCSS, when editing any styles you **must** edit the `.scss` files only. If any changes are made to the SCSS partials within `static/css/scss/`, you must re-compile with the following commands.
